...
> **Nota de reproducibilidad (relevante para lo que pide la guía).** Al revisar el archivo para armar esta respuesta noté que `comparison.json` y `lstm.json` reportan **aplanamientos distintos para el mismo modelo** (`lstm_c1`, serie Aérea, misma semilla=42, mismos epochs_usadas=500): `comparison.json` implica un nivel de cola de ~2,276,569 viajeros/mes (de ahí el MAE de 1.8 millones), mientras que la corrida guardada en `lstm.json` converge a ~68,547, un valor razonable y del mismo orden que los modelos clásicos. El `loss_final` tampoco coincide exactamente (0.0228 vs 0.0283). Esto sugiere que las dos corridas no son la misma ejecución pese a declarar la misma semilla — vale la pena re-correr y fijar esa fuente de no determinismo (por ejemplo, operaciones no deterministas de TensorFlow/cuDNN) antes de la entrega final, ya que el enunciado pide explícitamente que la investigación sea reproducible. La comparación de arriba usa los valores de `comparison.json` por ser el artefacto oficial de comparación, pero el número real de `lstm_c1` en Aérea podría no ser tan malo como aparenta si la corrida "buena" (la de `lstm.json`) es la representativa.
